...
export default function SmartPageAction({ action, appearance, preview }: { action: PageAction; appearance: ResolvedPresentation; preview: boolean }) {
  ...
  const className = appearance.v2
    ? "flex min-h-14 w-full min-w-0 items-center justify-center px-4 py-3 text-center text-base font-semibold leading-6 [overflow-wrap:anywhere] transition-colors hover:opacity-90 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4"
  ...
  const style = customStyle ? {
    ...
    minHeight: appearance.v2 ? 56 : 48, outlineColor: appearance.textColor,
  } : undefined;
  const content = <ActionContent label={action.label} iconKey={action.iconKey} imageSrc={action.imageSrc} />;
