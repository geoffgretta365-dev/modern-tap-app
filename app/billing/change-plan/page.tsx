...
export default async function ChangePlanPage() {
  ...
  return <AppShell businessName={business.name}><div className="mx-auto max-w-7xl"><Link href="/billing" className="text-sm underline">← Back to Billing</Link><h1 className="mt-5 text-3xl font-bold text-[#17324d]">Change Plan</h1><p className="mt-2 text-slate-600">Your current plan: {current?.name ?? "Legacy / custom subscription"}. {entitlement.configured ? `${entitlement.activeCount} plaques active.` : "Contact ModernTap to configure your allowance."}</p>
    <div className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{options.map(({ plan, price }) => {
      ...
      return <section key={plan.key} className="mt-panel p-5"><h2 className="text-xl font-bold">{plan.name}{current?.key === plan.key && <span className="mt-2 block text-sm text-[#0f766e]">Current Plan</span>}</h2><p className="mt-4 text-3xl font-bold">{displayPrice(plan)}<span className="text-sm font-normal"> / month</span></p><p className="mt-2">{plaqueRange(plan)}</p>{issue ? <p className="mt-5 text-sm text-slate-500">{issue}</p> : price.available && entitlement.configured ? <ChangePlanButton planKey={plan.key} name={plan.name}/> : <p className="mt-5 text-sm text-slate-500">Upgrade configuration unavailable.</p>}</section>;
