...
export default async function AnalyticsPage({ searchParams }: {
  ...
  const period: Period = requestedPeriod === "30" ? 30 : requestedPeriod === "90" ? 90 : 7;
  ...
  const performanceInsight = current === 0
    ? "No plaque taps were recorded during this period."
    : previous === 0
      ? `Your plaques recorded ${current.toLocaleString("en-US")} ${current === 1 ? "tap" : "taps"} during this period.`
      : current === previous
        ? `Taps were unchanged compared with the previous ${period}-day period.`
        : `Taps ${current > previous ? "increased" : "decreased"} ${changeLabel} compared with the previous ${period}-day period.`;
  ...
  return <AppShell businessName={business.name}>
    <div className="mx-auto max-w-[1280px]">
      ...
      <p className="mt-2 text-slate-500">See how often customers tap your plaques and which Smart Page actions they choose.</p>
      <p className="mt-1 text-xs text-slate-500">Dates and times are shown in Eastern Time.</p>

      <div className="mt-8 flex flex-wrap items-center justify-between gap-4">
        <p className="font-semibold text-[#17324d]">Last {period} Days <span className="font-normal text-sm text-slate-500">· including today</span></p>
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        <Card title="Taps" value={current} detail="Recorded opens of your plaque links in this period." />
        <Card title="Smart Page Actions" value={currentClicks} detail="Times customers selected an action on a Smart Page." />
      </div>
      <div className="mt-3 grid gap-2 px-1 text-sm text-slate-500 sm:grid-cols-2">
        <p>{comparison}</p><p>{clickComparison}</p>
      </div>
      <details className="mt-5 rounded-xl border border-[#dbe4ea] p-4">
        <summary className="cursor-pointer text-sm font-semibold text-[#17324d]">Tap totals across other periods</summary>
        <dl className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
          {[["Today", todayTaps], ["Last 7 Days", weekTaps], ["Last 30 Days", monthTaps], ["All Time", allTime]].map(([label, value]) => <div key={label}>
            <dt className="text-xs text-slate-500">{label}</dt><dd className="mt-1 text-lg font-semibold text-[#17324d]">{value.toLocaleString("en-US")}</dd>
          </div>)}
        </dl>
        <p className="mt-3 text-xs text-slate-500">Today starts at midnight Eastern Time. Day ranges include today.</p>
      </details>
      <div className="mt-5 px-1 text-sm leading-6 text-slate-600">
        <p>{performanceInsight}</p>
        {topPlaque && topPlaqueInteractions > 0 && <p className="break-words">{topPlaque.name} leads with {topPlaqueInteractions.toLocaleString("en-US")} taps.</p>}
      </div>

      <section className="mt-6 mt-panel min-w-0 p-5 sm:p-6">
        <h2 className="mt-section-heading text-lg">Taps Over Time</h2>
        <p className="mt-muted mt-1 text-sm">Daily plaque taps during the selected {period}-day period.</p>
        {current === 0
      ...
      <div className="mt-6 grid min-w-0 gap-6 lg:grid-cols-2">
        <section className="mt-panel min-w-0 p-5 sm:p-6">
          <h2 className="mt-section-heading text-lg">Top Plaques</h2>
          <p className="mt-muted mt-1 text-sm">Plaques ranked by taps in the selected period.</p>
          {ranked.length === 0
            ...
            : <div className="mt-5 divide-y divide-[#dbe4ea]">{ranked.map((plaque) => {
                ...
                return <div key={plaque.id} className="flex items-start justify-between gap-4 py-3">
                  <div className="min-w-0"><p className="break-words font-semibold text-slate-900">{plaque.name}</p>
                    <p className="mt-1 text-xs text-slate-500">{current ? (taps / current * 100).toFixed(1) : "0.0"}% of taps</p></div>
                  <div className="shrink-0 text-right"><p className="font-bold text-[#17324d]">{taps.toLocaleString("en-US")}</p>
                    <p className="text-xs text-slate-500">{taps === 1 ? "tap" : "taps"}</p></div>
                </div>;
              })}</div>}
        </section>
        <div className="grid gap-6">
          <section className="mt-panel min-w-0 p-5 sm:p-6">
            ...
            <p className="mt-muted mt-1 text-xs">The day with the most plaque taps in this period.</p>
            ...
            <p className="mt-muted mt-1 text-sm">{best ? `${best.taps.toLocaleString("en-US")} ${best.taps === 1 ? "tap" : "taps"}` : "No activity in this period"}</p>
          </section>
          <section className="mt-panel min-w-0 p-5 sm:p-6">
            <h2 className="mt-section-heading text-lg">Busiest Time</h2>
            <p className="mt-muted mt-1 text-xs">The hour of day with the most plaque taps in this period.</p>
            ...
            <p className="mt-muted mt-1 text-sm">{busiestHour === null ? "No activity in this period" : `${hours[busiestHour].toLocaleString("en-US")} ${hours[busiestHour] === 1 ? "tap" : "taps"} in this hour of day`}</p>
          </section>
        </div>
      </div>

      <section className="mt-6 mt-panel min-w-0 p-5 sm:p-6">
        <details><summary className="cursor-pointer text-lg font-semibold text-[#17324d]">All Plaque Details</summary>
        <p className="mt-muted mt-1 text-sm">Selected-period taps, lifetime totals, destinations, and status.</p>
        {plaques.length === 0
          ? <p className="mt-6 text-sm text-slate-500">No plaques yet. Add one to start tracking taps.</p>
      ...
      <section className="mt-6 mt-panel min-w-0 p-5 sm:p-6">
        <h2 className="mt-section-heading text-lg">Smart Page Actions</h2>
        <p className="mt-1 text-sm text-slate-500">Which actions customers choose, and where.</p>
        <dl className="mt-5 grid gap-4 rounded-xl bg-[#f3f7f9] p-4 sm:grid-cols-2">
          <div><dt className="text-xs text-slate-500">Actions in previous {period} days</dt><dd className="mt-1 text-lg font-semibold text-[#17324d]">{previousClicks.toLocaleString("en-US")}</dd></div>
          <div><dt className="text-xs text-slate-500">Actions compared with taps</dt><dd className="mt-1 text-lg font-semibold text-[#17324d]">{smartPageActionRate === null ? "—" : `${smartPageActionRate.toLocaleString("en-US", { maximumFractionDigits: 1 })}%`}</dd>
            <p className="mt-1 text-xs text-slate-500">Smart Page actions ÷ all plaque taps. Can exceed 100%; not a visit conversion rate.</p></div>
        </dl>
        <div className="mt-6 grid min-w-0 gap-6 lg:grid-cols-2">
          <div>
            <h3 className="font-semibold text-[#17324d]">Most Popular Actions</h3>
            <p className="mt-1 text-xs text-slate-500">Actions with the same name are grouped together.</p>
            {topButtons.length === 0
          ...
          <div className="rounded-2xl border border-[#dbe4ea] bg-white p-4 sm:p-5">
            <h3 className="font-semibold text-[#17324d]">Actions by Plaque</h3>
        ...
        <div className="mt-6 border-t border-slate-100 pt-5">
          <h3 className="font-semibold text-[#17324d]">Recent Smart Page Actions</h3>
          <p className="mt-1 text-xs text-slate-500">Latest actions, across all dates.</p>
      ...
      <section id="review-tracking" className="mt-6 mt-panel min-w-0 p-5 sm:p-6">
        <div className="mt-engagement-heading p-4 sm:p-5">
          ...
          <h2 className="mt-2 text-xl font-bold text-[#17324d]">Review Card Activity</h2>
          <p className="mt-1 text-sm text-slate-700">Taps on plaques marked as Review Cards.</p>
        </div>
        <div className="mt-5 grid grid-cols-2 gap-4 xl:grid-cols-4">
          {[["Today", reviewToday], ["Last 7 Days", reviewWeek], ["Last 30 Days", reviewMonth], ["All Time", reviewAllTime]].map(([label, value]) => <div key={label} className="rounded-xl bg-slate-50 p-4">
            <p className="text-xs text-slate-500">{label}</p><p className="mt-1 text-xl font-semibold text-[#17324d]">{value.toLocaleString("en-US")}</p>
          </div>)}
        </div>
        <p className="mt-5 rounded-xl bg-[#dffaf8] p-4 text-sm text-[#17324d]">These taps do not confirm that a review was submitted.</p>
        <div className="mt-6">
      ...
      <section className="mt-6 mt-panel min-w-0 p-5 sm:p-6">
        ...
        <p className="mt-muted mt-1 text-sm">Latest taps, across all dates.</p>
        {recent.length === 0
          ? <p className="mt-6 text-sm text-slate-500">No plaque activity yet. Recent taps will appear here.</p>
      ...
      <aside className="mt-6 border-t border-[#dbe4ea] px-1 pt-5 text-xs leading-5 text-slate-500" aria-labelledby="data-clarity-heading">
        ...
        <p className="mt-2">Plaque interactions show when a customer opens a ModernTap destination. Smart Page button clicks show tracked actions taken after opening a Smart Page. Review Card taps do not confirm submitted reviews, purchases, or revenue.</p>
      </aside>
    </div>
  </AppShell>;
}
