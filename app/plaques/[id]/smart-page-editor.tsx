...
type Page = { heading: string | null; subheading: string | null } | null;
...
export default function SmartPageEditor({ plaqueId, initialMode, page, buttons }: { plaqueId: string; initialMode: string; page: Page; buttons: Button[] }) {
  const router = useRouter();
  ...
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState("");
  useEffect(() => { setMode(initialMode); setHeading(page?.heading ?? ""); setSubheading(page?.subheading ?? ""); }, [initialMode, page]);
